...
def parse_thread(data: Dict) -> Dict:
    ...
    if result.get ("published_on"):
        try:
            dt = datetime.datetime.utcfromtimestamp(result["published_on"]) + datetime.timedelta(hours=7)
            result["published_on"] = dt.strftime("%d/%m/%Y %H:%M")
        except Exception:
            pass
...
if __name__ == "__main__":
    print(json.dumps(scrape_thread("https://www.threads.net/t/C8H5FiCtESk/"), indent=2, ensure_ascii=False))
